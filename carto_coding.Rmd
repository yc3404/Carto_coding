...
It seems people barely take taxi on Tuesdays and Sundays. Most taxi rides happen on Thursdays and Fridays. It is also strange that the data set does not contain observations on Mondays.
...
From the line plot, we can see the tip amount varies as time goes. The peak occurs at arround midnight, and the bottom occurs at 5:00 AM. It is also interesting to see the tip amount increases till he start of morning rush hour and decreases as rush hour proceeds. 
...
Because we have used the pickup and dropoff column to calculate the trip duration,then we can drop those two columns. Then I used the variable importance function to exclude some low importance features from the dataset: RatecodeID, store_and_fwd_flag, improvement_surcharge, tolls_amount, mta_tax, PULocationID, DOLocationID, weekday,day,passenger_count.
