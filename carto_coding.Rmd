...
Some columns need to be factored: VendorID, RatecodeID, Store_and_fwd_flag and payment_type. So we need to factor these columns for each dataset.
...
By insepecting the summary statistics, I noticed that several columns include some unreasonable outliers: fare_amount, total_amount,extra,mta_tax and tolls_amount.
```{r}
...
Because these outliers are very extreme, then we will remove values outside of three standard deviations of that column's average.
```{r}
trip_new <- trip_new %>% group_by(month) %>% 
  mutate(avg_fare = mean(fare_amount),
         sd_fare = sd(fare_amount),
         avg_total = mean(total_amount),
         sd_total = sd(total_amount),
         avg_extra = mean(extra),
         sd_extra = sd(extra),
         avg_mta = mean(mta_tax),
         sd_mta = sd(mta_tax),
         avg_toll = mean(tolls_amount),
         sd_toll = sd(tolls_amount),
         avg_dur = mean(duration),
         sd_dur = sd(duration)) %>%
  filter(fare_amount < avg_fare + 1.5*sd_fare,
         fare_amount > avg_fare - 1.5*sd_fare,
         total_amount < avg_total + 1.5*sd_total,
         total_amount > avg_total - 1.5*sd_total,
         extra < avg_extra + 1.5*sd_extra,
         extra > avg_extra - 1.5*avg_extra,
         mta_tax < avg_mta + 1.5*sd_mta,
         mta_tax > avg_mta - 1.5*sd_mta,
         tolls_amount < avg_toll + 1.5*sd_toll,
         tolls_amount > avg_toll - 1.5*sd_toll,
         duration < avg_dur + 1.5*sd_dur,
         duration > avg_dur - 1.5*sd_dur) %>%
  select(-c(avg_fare,sd_fare,avg_total,sd_total,avg_extra,sd_extra,avg_mta,sd_mta,
         avg_toll,sd_toll,avg_dur,sd_dur))%>%
  ungroup()
```
...
It seems people barely take taxi on Tuesdays and Sundays. Most taxi rides happen on Thursdays and Fridays.

I also noticed that people tend to be generous in giving tips in the middle of the week.

```{r}
trip_new$weekday <- factor(trip_new$weekday,
                           levels=c("Tuesday","Wednesday","Thursday",
                           "Friday","Saturday","Sunday"))
ggplot(trip_new,aes(weekday))+
  geom_bar(fill="#00b0ff")+
  geom_text(stat='count',aes(label=..count..),vjust=1)+
  labs(title="Histogram of Days in the Week\n",
       y = "Count",
       x = "Days in the Week")+
  theme_minimal()
...
I am curious about if weekends will affect tip amount. So I plot the average tip amount of weekends VS weekdays for March, June and November. Then I discovered in spring and summer, people tend to give less tips on weekends. But in winter, the tip amount is approximately the same for weekdays and weekends. 

```{r}
trip_new$month <- factor(trip_new$month,levels=c("March","June","Nov"))
weekend <- c("Saturday","Sunday")
trips <- trip_new %>% 
  mutate(weekend = ifelse(weekdays(tpep_pickup_datetime) %in% weekend,"Y","N"))%>%
...
From the line plot, we can see the tip amount varies as time goes. The peak occurs at arround midnight, and the bottom occurs at 5:00 AM. It is also interesting to see the tip amount increases to the start of morning rush hour and decreases as rush hour proceeds. 
...
trip_new %>% group_by(pickup_hour) %>% summarise(mean_tip = mean(tip_amount))%>%
  ...
  geom_point(color="#1565c0")+
  geom_line(color="#1565c0")+
  labs(title="Average Tip Amount per Pickup Hour\n",
       y = "Tip Amount ($)\n",
       x = "Pickup Hour")+
  theme_minimal()
```
...
I also found out the number of passenger seems not impacting tip amount a lot. The extreme high average tip amount for 9 people is because we have very few records of 9 passengers in a car. 
```{r}
#passenger count
...
ggplot(aes(x=passenger_count,y=mean_tip))+
  geom_point(color="#1565c0")+
  geom_line(color="#1565c0")+
  labs(title="Average Tip Amount per Passenger Count\n",
       y = "Tip Amount ($)",
       x = "Passenger Count")+
  theme_minimal()
```
...
From the plot we can see that there is a positive relationship between trip_distance and average tip amount. So we might use this column as a core feature. 
...
I also explored the relationship between trip duration and average tip amount. The relationship seems pretty positive until trip_duration greater than 40 minutes. I also added a trendline and it shows that the relationship is positive.
Therefore, I would use duration as a core feature.
...
Because we have used the pickup and dropoff column to calculate the trip duration,then we can drop those two columns. Then I used the variable importance function to exclude some low importance features from the dataset: RatecodeID, store_and_fwd_flag, improvement_surcharge, tolls_amount, mta_tax, PULocationID, DOLocationID, weekday,day and passenger_count.

The passenger_count's variable importance is around 6. But the previous data exploration shows that this feature does not play a large part, so I excluded it.
...
trip_new <- trip_new %>% select(-c(RatecodeID,store_and_fwd_flag,improvement_surcharge,mta_tax,                                   PULocationID,DOLocationID,day,tolls_amount,
                      weekday,passenger_count))
```
...
train_screen <- train_new[-influential,]
...
df <- tibble(Model=c("Baseline","Adjusted"),RMSE=c(RMSE_base,RMSE_lm),RMSE_mean=c(RMSE_mean_base,RMSE_mean_lm),R_squared=c(0.6423,0.7488))
